...
LOG_PATH  = "results/logs/noise/noise_results.json"
PLOT_PATH = "results/plots/noise/noise_distributions.png"
BAR_PATH  = "results/plots/noise/noise_performance_bar.png"
...
def main():
    ...
    for name, AgentClass in agent_classes.items():
        print(f"\nEvaluating {name} in noise environment (noise_std={NOISE_STD})...")
        env   = NoiseEnv(noise_std=NOISE_STD, seed=SEED)
    ...
    temp_log_paths = {}
    for name, stats in all_stats.items():
        path = f"results/logs/noise/{name.lower()}_noise_log.json"
